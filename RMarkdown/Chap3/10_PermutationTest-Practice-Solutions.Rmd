...
H~0~: The lead levels are the same in buried and surficial quaternary aquifers.
H~a~: The lead level in water from buried quaternary aquifers is less than the lead level in water from surficial quaternary aquifers.
...
```{r}
m <- length(buried) #length(vector) = number of values in vector
n <- length(surface)
total <- length(lead)
...
Technically, we would enter the command

```{r}
(sum(result <= observed) + 1)/(N+1)
```

However, we can see from the histogram of the permutation distribution is not normal at all.
Why is that? There are some extremely larger outliers in the dataset that are likely
misreadings. If we assume those entries are faulty, we can remove them and repeat the process.

```{r}
lead.clean <-  subset(MnGroundwater, select = Lead,
                      (Aquifer.Group == "buried Quaternary" | 
                         Aquifer.Group == "surficial Quaternary") & 
                        Lead < 80, drop = T)
...
pvalue.clean <- (sum(result.clean <= observed.clean) + 1)/(N+1)
pvalue.clean
```

e. **Interpret the meaning of the P-value.**

Let's make sense out of the clean version of this test. There is a `r pvalue.clean*100`% chance that we would get samples from buried and surficial quarternary aquifiers that have a mean lead levels with a difference as more or extreme than the observed difference of `r observed.clean` if there really is no difference in lead levels of water in the two different types of quarternary aquifiers.
