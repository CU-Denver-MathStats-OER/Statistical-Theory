...
Here is one possible bootstrap resample:

```{r}
sample(Diff, 12, replace = TRUE)
```

Here is another possible bootstrap resample:
...
There is a 95% chance the interval from `r lower` m/sec to `r upper` m/sec contains the actual mean of the difference in the maximum velocities of all people when the swim with and without the new wetsuit.

**Since 0 m/sec is not a value inside the interval estimate, it is NOT plausible that there is no difference in maximum velocities (since if that were the case, a difference of 0 m/sec would be plausible). In particular, since the interval estimate contains only positive values, the only plausible values for the difference are positive, and thus we see the wetsuit does help people swim at faster velocities.**

The mean of bootstrap distribution is `r mean.boot` m/sec and the bootstrap estimate for the standard error is `r se.boot` m/sec.
