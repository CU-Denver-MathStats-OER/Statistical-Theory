...
for (i in 1:N)
{
  ...
  boot.ratio.mean[i] <- mean(ILEC.sample)/mean(CLEC.sample)
}

lower <- quantile(boot.ratio.mean, probs = 0.025)
upper <- quantile(boot.ratio.mean, probs = 0.975)
```

Comparing the observed ratio to the mean of the sampling distribution for the ratio of mean repair times.

```{r}
original.ratio <- mean(Time.ILEC)/mean(Time.CLEC) #observed sample stat
mean.boot <- mean(boot.ratio.mean)    # Center of bootstrap

hist(boot.ratio.mean, main = "Bootstrap dist for ratio of means")
...
```{r}
qqnorm(boot.ratio.mean)
qqline(boot.ratio.mean)
```
...
We have $\mbox{E} \lbrack \hat{\theta}^{\ast} \rbrack =$ `r mean.boot`.

We have $\hat{\theta} =$ `r original.ratio`.

The bias is therefore $\mbox{Bias}_{\rm boot} \lbrack \hat{\theta}^{\ast} \rbrack =$ `r mean.boot-original.ratio`.
...
```{r}
(mean.boot - original.ratio)/sd(boot.ratio.mean)
```
