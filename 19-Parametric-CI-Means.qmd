...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://githubtocolab.com/CU-Denver-MathStats-OER/Statistical-Theory/blob/main/Chap5/19-Parametric-CI-Means.ipynb){target="_blank"} &nbsp;
...
- With <span style="color:dodgerblue">**parametric methods**</span>, we perform inference using formulas derived from theory to model the shape, center and spread of sampling distributions.
...
When practicing [exploratory data analysis earlier](03-EDA-Quantitative.qmd#sec-storms), we accessed the `dplyr` package that contains a data set from the [NOAA Hurricane Best Track Data](https://www.nhc.noaa.gov/data/#hurdat){target="_blank"}, called `storms`, with data on many different storm attributes. We will begin analyzing the variable `wind` that gives the wind speed in knots. Run the code cell below to: 
...
their.sample <- sample(??, size = ??, replace = ??)
```
...
A researcher collects a random sample of size $n=36$ of wind speeds for storms in the North Atlantic and calculates a 95% confidence
...
wind.lower90 <- ??   # Enter a command to calculate the lower limit
...
- Here is a [nice introduction](https://www.statology.org/tapply-r/){target="_blank"} to the `tapply()` function.
