...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://githubtocolab.com/CU-Denver-MathStats-OER/Statistical-Theory/blob/main/Chap1/03-EDA-Quantitative.ipynb) <nbsp>

# 1.3: Exploring Quantitative Data

------------------------------------------------------------------------

Additional Reading:

-   See [Overview of Plotting Data in R](Overview-of-Plots.qmd) for further reading and examples about plotting in R.

-   See [Fundamentals of Working with Data](Intro-to-Vectors-Dataframes.qmd) for further details about data types and logical statements in R.

-   [R Graph Gallery](https://r-graph-gallery.com/) has examples of many other types of graphs.

# Types of Variables

---

In statistics, <span style="color:dodgerblue">**variables**</span> are the attributes measured or collected in data. We refer to them as variables since the values or classes of attributes typically vary from observation to observation. The term variable is used differently in statistics from the notation of a variable in algebra. There are two types of variables in statistics:
...
In our work with [Exploring Categorical Data](02-EDA-Categorical.qmd), we performed an initial summary of the categorical variables in the `storms` data set. Today, we will investigate how to numerically and visually summarize quantitative variables.

# Getting to Know Our Data {#sec-storms}

------------------------------------------------------------------------

The `dplyr` package contains a data set from the [NOAA Hurricane Best Track Data](https://www.nhc.noaa.gov/data/#hurdat) that contains data on the following attributes of tracked North Atlantic storms since 1975:
...
See [Exploring Categorical Data](02-EDA-Categorical.qmd) for a refresher on our initial exploration with the `storms` data frame.

Then open the help documentation for `storms` in the second code cell to review the background of the data.
...
-   **By default, the counts are right closed.**
...
- The <span style="color:dodgerblue">**median**</span> is the $50^{\mbox{th}}$ percentile. This means 50% of the values in the data set are less than the median. 
  - In R, we use the function `median()`. - If there are an odd number of values, the median is the middle value. 
...
*Hint: We can input the vector of wind speeds with the code `storms$wind`.*
...
A question we often wish to explore is what proportion of values in our data are less or equal to a specified value $x$? To answer this question, we count to the total number of observations in our data that are less than or equal to $x$, and then divide by the total number of observations in our data.
...
We can compare data for only August and September using various methods. One common method is to subset all of the data in `storms` into two separate data frames, one for each month. Below three are different ways we can subset data:
